...
import(
	"shop/pre_shop/models/class" // 注册模型，需要引入该包
...
func init(){
	...
	orm.RegisterModel(new(class.User),new(class.Goods_category),new(class.Goods),new(class.Image),new(class.Cart),new(class.Order)) // 注册模型，建立User类型对象，注册模型时，需要引入包
